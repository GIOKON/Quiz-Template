...
endpoint;
  ...
  constructor(private eRef: ElementRef, auth: AuthService, private apiService: ApiService) {
    if (window.innerWidth <= 767) {
      this.mobileMode = true;
      this.desktopMode = false;
    } else {
    ...
    if (auth.loggedIn()) {
      this.apiService.FindEndpoint().subscribe((model: any) => {
        this.endpoint = model.values;
        console.log(model);
